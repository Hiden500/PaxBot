...
## Версия 3.5 — Архитектурный рефакторинг (текущий спринт)

**Цель:** Устранить технический долг, избавиться от God-файлов и дублирования кода, добавить поддержку кастомных OpenAI-совместимых эндпоинтов.

- [ ] **Phase 1 (P1):** Универсализация провайдеров & OpenAI Compatible API
...
- [ ] **Phase 2 (P1):** Декомпозиция `src/index.ts` (God file)
  - Выделение логики загрузки браузера в `src/boot.ts`
  - Выделение когнитивного цикла в `src/loop.ts`
  - Оптимизация чтения из файловой системы (устранение IO-in-loop)
- [ ] **Phase 3 (P2):** Рефакторинг `src/shared/tui.ts`
  - Разделение на `StateStore` и `TerminalRenderer`
- [ ] **Phase 4 (P2):** Упрощение сложных модулей
