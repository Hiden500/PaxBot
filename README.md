...
### 4. Выбор языка бота (Опционально)

По умолчанию бот генерирует свои мысли и действия (в игре) на английском. Для переключения на русский или другой язык добавьте в `.env`:

```env
AGENT_LANGUAGE=Russian
```

### 5. Захватить состояние аутентификации
...
При запуске без флагов откроется **Интерактивное стартовое меню**:

1. **Continue**: Запуск когнитивного цикла для выбранной кампании по сохранённому для неё URL.
2. **Select Campaign**: Переключение между доступными кампаниями.
3. **Create New Campaign**: Создание новой кампании на базе Markdown-шаблона. Бот создаст шаблон в `war-room/campaigns/`, подождёт вашего редактирования и автоматически скомпилирует JSON-файл и Strategy Plan через LLM.
4. **Change GAME_URL**: Привязка игрового URL к текущей кампании (сохраняется в сессию).
5. **View Campaign Status**: Вывод ключевых достижений, ошибок, уроков и активных задач в консоль.
6. **Reset Campaign Session**: Очистка прогресса сессии.

### Опции запуска

```powershell
# Запуск в обход стартового меню (продолжить с текущими настройками)
npm start -- --continue

# Задать URL игры в обход сохранения в кампанию
