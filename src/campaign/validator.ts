...
function validateStringFields(c: Record<string, unknown>, errors: string[]) {
...
function validatePriorities(c: Record<string, unknown>, errors: string[], warnings: string[]) {
  if (!Array.isArray(c.priorities)) {
    errors.push(`"priorities" must be an array`);
    return;
  }

  if (c.priorities.length === 0) {
...
function validateConstraints(c: Record<string, unknown>, errors: string[]) {
...
function validateVictoryConditions(
  ...
  if (c.victoryConditions.length === 0) {
...
export function validateCampaign(campaign: unknown): ValidationResult {
  ...
  validateStringFields(c, errors);

  if (typeof c.timeHorizon !== "number" || c.timeHorizon < 1) {
    errors.push(`"timeHorizon" must be a positive number, got ${String(c.timeHorizon)}`);
  }

  validatePriorities(c, errors, warnings);
  validateConstraints(c, errors);
  validateVictoryConditions(c, errors, warnings);
